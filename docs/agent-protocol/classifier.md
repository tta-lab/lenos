...
5. Lenos Bash tag parse: extract bash blocks or re-prompt on invalid tag
   syntax.
6. Natural-language emit: end the turn as Markdown prose.
7. mvdan Bash syntax check: re-prompt on syntax failure.
8. Everything else inside bash blocks: execute as bash.

Markdown prose outside bash blocks is valid reader-facing output.
