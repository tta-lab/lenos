...
func TestCrushIgnore(t *testing.T) {
	...
	// Create a .lenosignore file that ignores .log files
	require.NoError(t, os.WriteFile(".lenosignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .lenosignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .lenosignore in subdir that ignores normal_nested
	subCrushignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".lenosignore"), []byte(subCrushignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .lenosignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .lenosignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .lenosignore")
