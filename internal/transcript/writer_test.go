...
func TestMdWriter_AppendStrict_Happy(t *testing.T) {
	t.Parallel()
	...
	err := w.AppendStrict([]byte("hello world\n"))
	require.NoError(t, err)
...
func TestMdWriter_AppendStrict_OpenFailure(t *testing.T) {
	t.Parallel()
	...
	w := NewMdWriter(path)
	err = w.AppendStrict([]byte("won't be written\n"))
	require.Error(t, err)
	require.Contains(t, err.Error(), "open")
	require.Contains(t, err.Error(), path)
}

func TestMdWriter_AppendStrict_ConcurrentWrite(t *testing.T) {
	t.Parallel()
	...
	w := NewMdWriter(path)
	err = w.AppendStrict([]byte("blocked\n"))
	require.ErrorIs(t, err, ErrConcurrentWrite)
}
