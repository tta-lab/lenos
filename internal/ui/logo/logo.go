...
func Render(s *styles.Styles, version string, compact bool, o Opts) string {
	...
	lenos := renderWord(spacing, stretchIndex, letterforms...)
	lenosWidth := lipgloss.Width(lenos)
	b := new(strings.Builder)
	for r := range strings.SplitSeq(lenos, "\n") {
	...
	// Charm and version.
	metaRowGap := 1
	maxVersionWidth := lenosWidth - lipgloss.Width(charm) - metaRowGap
	version = ansi.Truncate(version, maxVersionWidth, "…") // truncate version if too long.
	gap := max(0, lenosWidth-lipgloss.Width(charm)-lipgloss.Width(version))
	metaRow := fg(o.CharmColor, charm) + strings.Repeat(" ", gap) + fg(o.VersionColor, version)

	// Join the meta row and big Lenos title.
	lenos = strings.TrimSpace(metaRow + "\n" + lenos)

	// Narrow version.
	if compact {
		field := fg(o.FieldColor, strings.Repeat(diag, lenosWidth))
		return strings.Join([]string{field, field, lenos, field, ""}, "\n")
	}

	fieldHeight := lipgloss.Height(lenos)
	...
	// Right field.
	rightWidth := max(15, o.Width-lenosWidth-leftWidth-2) // 2 for the gap.
	const stepDownAt = 0
	...
	const hGap = " "
	logo := lipgloss.JoinHorizontal(lipgloss.Top, leftField.String(), hGap, lenos, hGap, rightField.String())
	if o.Width > 0 {
...
func SmallRender(t *styles.Styles, width int) string {
	...
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad(t, "Lenos", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "Lenos"
	if remainingWidth > 0 {
