...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "lenos://skills/builtin-skill/SKILL.md", Builtin: true},
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "lenos-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "lenos://skills/lenos-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "lenos://skills/lenos-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "lenos-config builtin skill not found")
}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "lenos-config", Path: "lenos://skills/lenos-config"}, {Name: "lenos-config", Path: "/user/lenos-config"}},
			wantLen:  1,
			wantName: "lenos-config",
			wantPath: "/user/lenos-config",
		},
		{
			name:    "empty",
			input:   nil,
