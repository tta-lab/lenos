...
// BashOutputConfig controls bash command output bounding.
type BashOutputConfig struct {
	MaxLines int `json:"max_lines,omitempty" jsonschema:"description=Maximum lines before truncating,default=2000,minimum=20"`
	MaxBytes int `json:"max_bytes,omitempty" jsonschema:"description=Maximum bytes before truncating,default=51200,minimum=1024"`
}
...
type Options struct {
	...
	BashOutput                *BashOutputConfig `json:"bash_output,omitempty" jsonschema:"description=Output bounding for bash commands"`
}
...
// Config holds the configuration for lenos.
type Config struct {
