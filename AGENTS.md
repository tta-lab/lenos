...
`Summarize()` in `agent_session.go` ALWAYS uses `a.largeModel.Get()` —
invariant: summaries are generated by the large tier regardless of
`ActiveTier` (preserves quality even when user runs the coder on small).
...
explicit persistence. Summarize always uses the large model regardless of
`ActiveTier` — this is an invariant.
